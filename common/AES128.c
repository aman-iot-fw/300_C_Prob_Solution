...
#define AES128_KYE_SIZE 16
#define BLOCK_SIZE      16


uint8_t *plainText = "EmbeddedEngineer";
uint8_t *key = "EngineerEmbedded";
uint8_t encryptedText[BLOCK_SIZE];
uint8_t output[BLOCK_SIZE];

void aes128encrypt(uint8_t *plainText, uint8_t *key, uint8_t *encryptedText){
  for(int i = 0; i < BLOCK_SIZE; i++){
    encryptedText[i] = plainText[i] ^ key[i];
  }
}

void aes128decrypt(uint8_t *output, uint8_t *key, uint8_t *encryptedText){
  for(int i = 0; i < BLOCK_SIZE; i++){
    output[i] = encryptedText[i] ^ key[i];
  }
}
...
int main(){
  ...
  aes128encrypt(plainText, key, encryptedText);
  printBuffer("Encrypted Text", encryptedText, BLOCK_SIZE);
  aes128decrypt(output, key, encryptedText);
